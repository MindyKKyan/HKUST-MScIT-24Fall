...
"""
Student ID: 21108128
Student Name: Mingzhen JIANG
Student Email: mjiangar@connect.ust.hk
Assignment #1
...
def compute_softmax_loss(W, X, y, reg):
    ...
    num_samples = len(X)  # Number of examples

    # Calculate scores (X * W)
    scores = np.dot(X, W)
    
    # First term of the loss function
    correct_class_scores = np.sum(scores[np.arange(num_samples), y])
    
    # Second term of the loss function
    log_sum_exp = np.sum(np.log(np.sum(np.exp(scores), axis=1)))
    
    # Compute total loss with regularization
    loss = -(correct_class_scores - log_sum_exp) / num_samples + 0.5 * reg * np.sum(np.power(W, 2))

    # Gradient calculation
    gradient_first = np.zeros((num_samples, W.shape[1]))
    gradient_first[np.arange(num_samples), y] = 1
    
    # Compute softmax probabilities
    softmax_denom = np.sum(np.exp(scores), axis=1)
    softmax_probs = np.exp(scores) / softmax_denom[:, None]

    # Calculate the gradient of the W
    gradient_diff = gradient_first - softmax_probs
    dW = -np.dot(X.T, gradient_diff) / num_samples
